...
- `timesfm/` - các thử nghiệm mô hình, tiện ích kiểm định và các script sinh output
- `submission*.csv` - các file nộp bài đã lưu
- `visual_outputs/` ở thư mục gốc - bảng kiểm định, feature importance, SHAP outputs và chẩn đoán mô hình
...
- `timesfm/timesfm_tuned_search.py` - tinh chỉnh trọng số TimesFM trên các fold kiểm định và sinh submission cuối `submission_best_timesfm_tuned.csv`
...
Script này tổng hợp nhiều mô hình, tạo `submission_best_meta_ensemble.csv`, và ghi các file kiểm định trong `visual_outputs/`.

### 9. Chạy TimesFM tuned search để sinh submission cuối

```bash
python timesfm/timesfm_tuned_search.py
```

Script này cần chạy sau khi đã có:
...
- `submission_best_timesfm_tuned.csv`
...
Script sẽ tự kiểm tra file submission sinh ra có khớp với `sample_submission.csv`.
...
- `visual_outputs/timesfm_tuned_*.csv`
