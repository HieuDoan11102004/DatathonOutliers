...
def train_linear(
    ...
    for epoch in range(epochs):
        ...
        history_vl_loss.append(vl_loss)
    ...
    try:
        ...
        plt.plot(history_vl_loss, label='Val Loss', color='orange')
        plt.title(f'{model_name} Training Loss')
        plt.xlabel('Epochs')
        plt.ylabel('Huber Loss')
        plt.legend()
        plt.grid(True)
        out_path = f'plot_{model_name}_loss.png'
        plt.savefig(out_path)
        plt.close()
        print(f"      [PLOT] Saved loss plot to {out_path}")
    except Exception as e:
        print(f"      [PLOT] Could not plot loss: {e}")
