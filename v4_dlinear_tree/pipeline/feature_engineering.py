...
} ## CHECK GIẢM GIÁ CÓ PHẢI GIỖ TỔ KHÔNG
...
def add_lag_features(df):
    ...
    for lag in lag_days:
        new_cols[f'rev_lag_{lag}'] = rev.shift(lag)

    # ── Raw COGS lags (stable signal, phản ánh mix sản phẩm) ─
    for lag in [548, 728, 912]:
        new_cols[f'cogs_lag_{lag}'] = cogs.shift(lag)
    ...
    for lag_base in [548, 728, 912]:
        ...
        for window in [7, 14, 30, 60, 90]:
            ...
            new_cols[f'rev_rmean_{lag_base}_{window}'] = shifted_rev.rolling(window, min_periods=min_p).mean()
            if window in [30, 90]:   # std chỉ cần cửa sổ lớn
                new_cols[f'rev_rstd_{lag_base}_{window}'] = shifted_rev.rolling(window, min_periods=min_p).std()
        new_cols[f'cogs_rmean_{lag_base}_30'] = shifted_cogs.rolling(30, min_periods=5).mean()
    ...
    for (l1, l2) in [(548, 912), (728, 1092), (912, 1456)]:
        r1 = new_cols[f'rev_lag_{l1}']
        r2 = new_cols[f'rev_lag_{l2}']
        safe_r2 = r2.replace(0, np.nan)
        new_cols[f'rev_yoy_ratio_{l1}_{l2}']  = (r1 / safe_r2).clip(0, 5)
        new_cols[f'rev_yoy_delta_{l1}_{l2}']  = (r1 - r2) / (r2.abs() + 1)

    # COGS/Revenue ratio (gross margin proxy từ lag)
    for lag in [548, 728]:
        r = new_cols[f'rev_lag_{lag}']
        c = new_cols[f'cogs_lag_{lag}']
        safe_r = r.replace(0, np.nan)
        new_cols[f'gross_margin_lag_{lag}'] = ((r - c) / safe_r).clip(-1, 1)
        new_cols[f'cogs_ratio_lag_{lag}']   = (c / safe_r).clip(0, 2)
    ...
    annual_mean_safe = new_cols.get('rev_rmean_548_90', pd.Series(np.nan, index=df.index)).replace(0, np.nan)
    new_cols['rev_seasonality_idx'] = (new_cols['rev_lag_548'] / annual_mean_safe).clip(0, 5)
...
def add_promotion_features(df):
    ...
    new_cols = {}
    new_cols['promo_n_active']          = n_active_all
    new_cols['promo_n_percentage']      = n_active_pct
    new_cols['promo_n_fixed']           = n_active_fixed
    new_cols['promo_max_discount']      = max_discount
    new_cols['promo_sum_discount']      = sum_discount
    new_cols['promo_has_stackable']     = has_stackable
    new_cols['promo_n_channels']        = n_channels
    new_cols['promo_min_order_thresh']  = min_order_threshold
    new_cols['promo_is_active']         = (n_active_all > 0).astype(int)

    # Promo intensity score (kết hợp số lượng + giá trị)
    new_cols['promo_intensity'] = (
        new_cols['promo_n_active'] * 0.4 +
        np.clip(new_cols['promo_max_discount'] / 100.0, 0, 1) * 0.6
    )

    print(f"  Promotion features added — coverage: "
          f"{new_cols['promo_is_active'].mean()*100:.1f}% of days have active promo")
...
def build_features(sales_df=None):
    ...
    print("[7/7] Interaction Features removed by Signal Pruning strategy...")
